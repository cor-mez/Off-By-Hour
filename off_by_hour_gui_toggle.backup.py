...
def extract_timeoff_events(to_csv: Path, week_start: date, week_end: date):
    ...
    datetime_c = next((cols[k] for k in cols if "date and time" in k or "dates" in k), None)
    date_col   = next((cols[k] for k in cols if re.search(r'\bdate\b', k)), None) if datetime_c is None else None
        ...
        for _, r in df.iterrows():
            ...
            if "Through" in spec:
                ...
                if re.search(r'all\s*day', spec, flags=re.I):
                    add_event(raw_name, d, 0, 24*60 + 60)
                else:
                    m = re.search(r'(\d{1,2}(:\d{2})?\s*(AM|PM))\s*-\s*(\d{1,2}(:\d{2})?\s*(AM|PM))', spec, flags=re.I)
                    if m:
                        smin = parse_time_flexible(m.group(1)); emin = parse_time_flexible(m.group(4))
                        if smin is None or emin is None: continue
                        emin = min(emin + 60, 24*60)  # inclusive end hour
                        add_event(raw_name, d, smin, emin)
                    else:
                        add_event(raw_name, d, 0, 24*60 + 60)
    else:
        # If separate date columns appear (rare in your files)
        for _, r in df.iterrows():
            raw_name = str(r.get(name_col,"")).strip()
            if not raw_name: continue
            d = None
            if date_col and pd.notna(r.get(date_col)):
                try: d = pd.to_datetime(r[date_col]).date()
...
def build_timeoff_sum(avail_csv: Path, timeoff_csv: Path, week_monday: date|None):
    ...
    def map_to_availability(foh_name: str):
        f_first, f_last = split_first_last(foh_name)
    ...
    mappings = []
    name_to_avail = {}
    for raw_name, _, d, smin, emin in events:
...
def main():
    ...
    if to_path.suffix.lower() == ".pdf":
        outdir = Path(getattr(args, "outdir", "./out"))
        to_path = convert_timeoff_pdf_to_csv(to_path, outdir / "TimeOff_from_PDF.csv")
    else:
        outdir = Path(getattr(args, "outdir", "./out"))

    sum_df, mappings, wk_start, wk_end = build_timeoff_sum(
        Path(args.availability), to_path, week_monday
    )
    ...
    print(f"\nSaved:\n- {xlsx}\n- {csv}\n- {map_csv}\n")
        ...
        to_path = Path(args.timeoff)
        # NEW: if a PDF is selected, convert it to CSV in the output folder
        if to_path.suffix.lower() == ".pdf":
            to_path = convert_timeoff_pdf_to_csv(to_path, outdir / "TimeOff_from_PDF.csv")
